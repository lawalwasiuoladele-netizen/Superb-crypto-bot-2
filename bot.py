...
TRADE_TIMEOUT = int(os.environ.get("TRADE_TIMEOUT", str(20 * 60)))  # 20 minutes
CHECK_INTERVAL = int(os.environ.get("CHECK_INTERVAL", "10"))  # seconds
MIN_USDT_BALANCE = float(os.environ.get("MIN_USDT_BALANCE", "3.0"))
...
class BotController:
    ...
    def remove_account_index(self, idx: int):
        accounts = self.load_accounts()
    ...
    def _should_sell(self, entry_time, entry_price, current_price):
        ...
        # Take profit windows
        if elapsed <= 10:   # first 10 minutes
            return profit_pct >= 10.0
        if elapsed <= 16:   # next 6 minutes
            return profit_pct >= 5.0
        if elapsed <= 20:   # last 4 minutes
        ...
        try:
            ...
            while not self._stop_event.is_set():
                ...
                if self._should_sell(entry_time, entry_price, price) \
                        or time.time() - entry_time > TRADE_TIMEOUT:
