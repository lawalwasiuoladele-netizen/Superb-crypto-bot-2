...
import queue   # ✅ use queue.Queue for thread-safe queues
...
RISK_RULES = {
    ...
    "tp1": (600, 7.0),    # first 10 minutes -> 7% (600s)
    "tp2": (1020, 4.0),   # up to 17 minutes -> 4% (1020s)
    "tp3": (1200, 1.0),   # up to 20 minutes -> 1% (1200s)
...
class BotController:
    ...
    def log(self, msg: str):
        ...
        if self.log_queue:
            try:
                # non-blocking attempt to put log into queue for server UI
                self.log_queue.put(line, block=False)
            except Exception:
                # swallow queue problems so logging never crashes the bot
                pass
    ...
    def get_klines(self, session: HTTP, symbol: str, interval: str = "1", limit: int = 50) -> List[Dict[str, Any]]:
        try:
            ...
            if not candles:
                return []
            # returned list is usually newest-first; keep oldest-first for indicators
    ...
    def get_balance(self, session: HTTP, acc: Dict[str, Any], retries: int = 2) -> Optional[float]:
        for attempt in range(retries):
            try:
                ...
                # result format varies: take totalEquity if present, else sum of wallets
                if balance_list:
                    first = balance_list[0]
                    total = float(first.get("totalEquity", 0)) if first.get("totalEquity") is not None else 0.0
    ...
    def check_entry_condition(self, symbol: str, closes: List[float], current_price: float, volumes: Optional[List[float]] = None) -> bool:
        try:
            ...
            # signal if RSI is notably away from neutral OR extreme (oversold/overbought)
