"""
bot.py — Superb Crypto Bot (robust, recursion-safe)

Key fixes/choices in this file:
 - is_running() present (used by server.py)
 - TP timeline + elapsed tracking as requested:
     TP1: 0-10m -> +7%
     TP2: 10-18m -> +4%
     TP3: 18-23m -> +1%
     Stop-loss = -3%, forced exit after 23m
 - _parse_price includes depth guard to avoid infinite recursion
 - _try_methods has local 'seen' protections and checks callability
 - add_trade/update_trade sanitize records via json.dumps(default=str)
 - Thread-safe file writes, dry_run default ON
 - Safe SDK fallbacks; minimal assumptions about response shapes
"""
...
# TP durations/targets and stop rules (seconds)
RISK_RULES = {
    "stop_loss": -3.0,
    "tp1": (600, 7.0),     # 0-600s -> 7%
    "tp2": (1080, 4.0),    # 600-1080s -> 4%
    "tp3": (1380, 1.0),    # 1080-1380s -> 1%
    "max_hold": 1380,
...
TRADE_SETTINGS = {
    ...
    "dry_run": True             # safe default (simulated orders)
}
...
class BotController:
    def __init__(self, log_queue: Optional[threading.Queue] = None):
        ...
        # Ensure files exist
        for p in (ACCOUNTS_FILE, TRADES_FILE):
    ...
    def is_running(self) -> bool:
        """Return True when bot is active and not stopping."""
    ...
    def add_trade(self, trade: Dict[str, Any]):
        """Sanitize trade record before saving so non-serializable / circular objects become strings."""
        with self._file_lock:
            trades = self._read_trades()
            try:
                # sanitize using json.dumps default=str to avoid serialization errors/circular refs
                sanitized_json = json.dumps(trade, default=str)
                sanitized = json.loads(sanitized_json)
            except Exception:
                # fallback simpler sanitize: str() all non-primitive values
                sanitized = {}
    ...
    def update_trade(self, trade_id: str, updates: Dict[str, Any]) -> bool:
        """Update a trade with sanitized update values."""
        with self._file_lock:
            trades = self._read_trades()
            changed = False
            for t in trades:
                if t.get("id") == trade_id:
                    # sanitize updates similarly
                    try:
                        sanitized_json = json.dumps(updates, default=str)
                        sanitized = json.loads(sanitized_json)
                    except Exception:
                        sanitized = {k: (v if isinstance(v, (str, int, float, bool, type(None))) else str(v)) for k, v in updates.items()}
                    t.update(sanitized)
                    changed = True
                    break
            if changed:
    ...
    def _capture_preview(self, account: Dict[str, Any], resp: Any):
        """Keep small preview of responses (when debug enabled) — avoids secrets."""
        try:
            if not TRADE_SETTINGS.get("debug_raw_responses", False):
                return
            try:
                preview = json.dumps(resp, default=str)[:300]
            except Exception:
    ...
    def _try_methods(self, client: HTTP, candidate_names: List[str], *args, **kwargs):
        """Try candidate method names on client; avoid infinite fallback loops and ensure callability."""
        last_exc = None
        seen_ids = set()
        for name in candidate_names:
            try:
                ...
                if resp is None:
                    continue
                # If response is dict and we've already seen this object's id in this call, skip
    ...
    # ---- ticker/klines and price parsing (depth-guarded) ----
    ...
    def _parse_price(self, raw: Any, depth: int = 0) -> Optional[float]:
        """Extract numeric price from many possible shapes safely. Depth-limited to prevent recursion."""
        if depth > 4:
            return None
        try:
            if raw is None:
            ...
            if isinstance(raw, dict):
                ...
                for container in ("result", "data"):
                    v = raw.get(container)
                    if v is not None:
                        p = self._parse_price(v, depth + 1)
                        if p is not None:
                            return p
                # look for list-like fields
                for subk in ("list", "ticks", "rows", "data"):
                    v = raw.get(subk)
                    if isinstance(v, (list, tuple)) and v:
                        p = self._parse_price(v[0], depth + 1)
                        if p is not None:
                            return p
            if isinstance(raw, (list, tuple)) and raw:
                # kline-like arrays: [ts, open, high, low, close, ...]
                first = raw[0]
                # if we have a nested list as the first element (list of klines)
                if isinstance(first, (list, tuple)) and len(first) >= 5:
                    try:
                        return float(first[4])
                    except Exception:
                        pass
                # if array length >=5 and looks like kline
                if len(raw) >= 5 and all(isinstance(x, (int, float, str)) for x in raw[:5]):
                    try:
                        return float(raw[4])
                    except Exception:
                        pass
                # otherwise try first element recursively
                return self._parse_price(first, depth + 1)
            return None
        except Exception:
            return None
    ...
    def _place_market_order(self, client: HTTP, symbol: str, side: str, qty: float, price_hint: Optional[float] = None) -> Dict[str, Any]:
        if TRADE_SETTINGS.get("dry_run", True):
            self.log(f"[DRY RUN] simulate {side} {symbol} qty={qty}")
    ...
    def _finalize_trade(self, trade_id: str, exit_price: float, exit_ts: int, label: str, resp_summary: Optional[str], simulated: bool):
        ...
        try:
            ...
            # handle trailing Z or other formats
            try:
    ...
    def attempt_trade_for_account(self, acct: Dict[str, Any]):
        ...
        try:
            ...
            if price is None:
                self.log(f"Could not determine price for allocation — aborting open for {symbol}")
                return
