...
- Auto-pick best symbol from whitelist
- Time-based profit targets + stop loss
- Force-sell at 20m
- 50 trades/day cap
...
# ✅ Whitelisted coins (only these will be traded)
ALLOWED_COINS = {
    "XRPUSDT", "XLMUSDT", "HBARUSDT",
    "VETUSDT", "TRXUSDT", "DOGEUSDT",
    "ADAUSDT", "SOLUSDT", "MATICUSDT"
}
...
MIN_USDT_BALANCE = float(os.environ.get("MIN_USDT_BALANCE", "3.0"))  # lowered to $3
RATE_LIMIT_SLEEP = float(os.environ.get("RATE_LIMIT_SLEEP", "0.35"))
MAX_RETRIES = int(os.environ.get("MAX_RETRIES", "3"))
TESTNET = False  # 🚨 Real trading (ensure API keys & permissions are correct)
...
class BotController:
    ...
    def _get_ticker_last(self, session, symbol):
        try:
    ...
    def _get_symbol_step(self, session, symbol):
        try:
    ...
    # ---------- symbol selection ----------
    def _pick_allowed_coin(self, session):
        try:
            tickers = session.get_tickers(category="spot")["result"]["list"]
            candidates = []
            for t in tickers:
                sym = t.get("symbol")
                if sym not in ALLOWED_COINS:
                    continue
                price = float(t.get("lastPrice", 0))
                vol = float(t.get("turnover24h", 0))
                candidates.append((sym, price, vol))
    ...
    def _should_sell(self, entry_time, entry_price, current_price):
        ...
        if elapsed <= 10 and profit_pct >= 10:
            return True
        if elapsed <= 16 and profit_pct >= 5:
            return True
        if elapsed <= 20 and profit_pct >= 2:
            return True
    ...
    def _run_one_trade(self, sessions):
        try:
            first = sessions[0][0]
            symbol = self._pick_allowed_coin(first)
            if not symbol:
                self.log("No allowed coin available this round.")
                return False

            entry_price = self._get_ticker_last(first, symbol)
            if not entry_price: return False
            step = self._get_symbol_step(first, symbol)
            self.log(f"Entry {entry_price:.4f} {symbol}, step {step}")
            ...
            for sess, acc in sessions:
                ...
                if self._place_market_order(sess, symbol, "Buy", qty):
                    ...
                    self.log(f"{acc['key'][:8]} BUY {qty} {symbol} (~{usdt:.2f} USDT)")
            ...
            while not self._stop_event.is_set():
                ...
                price = self._get_ticker_last(first, symbol)
                if not price: continue
                self.log(f"{symbol} {price:.4f}, elapsed {int(time.time()-entry_time)}s")

                if self._should_sell(entry_time, entry_price, price):
                    for sess, acc, qty in buys:
                        self._place_market_order(sess, symbol, "Sell", qty)
                        self.log(f"{acc['key'][:8]} SOLD {qty} {symbol} at {price:.4f}")
                    return True
            return False
        except Exception as e:
