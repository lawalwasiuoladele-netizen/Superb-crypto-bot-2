"""
bot.py — Superb Crypto Bot (final patched version)

- Adds is_running() to avoid AttributeError from server.py
- TP timeline: TP1 (0-10m -> +7%), TP2 (10-18m -> +4%), TP3 (18-23m -> +1%)
- Stop-loss: -3%, forced exit after 23 minutes
- Records trades with elapsed (Mm Ss) and profit% to trades.json
- Dry-run default ON (safe). Toggle TRADE_SETTINGS['dry_run'] = False to actually place orders.
- Thread-safe file updates and robust SDK fallbacks.
"""
...
# TP durations and percentages (end_time_seconds, percent)
RISK_RULES = {
    "stop_loss": -3.0,                 # -3% stop loss
    "tp1": (600, 7.0),                 # 0 -> 600s (10m) -> 7%
    "tp2": (1080, 4.0),                # 600 -> 1080s (10m-18m) -> 4%
    "tp3": (1380, 1.0),                # 1080 -> 1380s (18m-23m) -> 1%
...
TRADE_SETTINGS = {
    ...
    "dry_run": True                 # default: simulate orders (safe)
}
...
class BotController:
    ...
    def _parse_price(self, raw: Any) -> Optional[float]:
        try:
            ...
            if isinstance(raw, dict):
                for k in ("price", "last_price", "lastPrice", "close", "last"):
                    if k in raw:
                        try:
                            return float(raw[k])
                        except Exception:
                ...
                for container in ("result", "data"):
                    ...
                    if isinstance(v, dict):
                        for k in ("price", "last_price", "lastPrice", "close"):
                            if k in v:
                                try:
                                    return float(v[k])
                                except Exception:
            ...
            if isinstance(raw, (list, tuple)) and raw:
                ...
                if len(raw) >= 5:
                    try:
                        ...
                        pass
                return self._parse_price(raw[0])
            return None
        except Exception:
            return None
    ...
    # ---- trading flow: open / monitor / close ----
    def attempt_trade_for_account(self, acct: Dict[str, Any]):
        ...
        if bal < 10.0:
            ...
            return
        # pick symbol and allocation
        symbol = acct.get("preferred_symbol") or ALLOWED_COINS[0]
        alloc_pct = TRADE_SETTINGS.get("trade_allocation_pct", TRADE_SETTINGS.get("trade_allocation_pct", 0.01))
        usd_alloc = bal * alloc_pct
        if usd_alloc < 1.0:
        ...
        try:
            ...
            if price is None:
                self.log(f"Could not parse price for {symbol} — aborting open")
                return
