"""
Final patched bot.py — recursion-safe, TP-timed, production-ready (dry-run safe)

Features:
 - is_running()
 - TP1 (0-10m -> +7%), TP2 (10-18m -> +4%), TP3 (18-23m -> +1%)
 - Stop-loss -3%, forced exit after 23 minutes
 - Elapsed time recorded in minutes:seconds and seconds
 - Robust, non-recursive parsing of exchange responses
 - Safe file I/O with sanitization for trades
 - Dry-run default = True
 - test_on_testnet default = False (production endpoint)
"""
...
TRADE_SETTINGS = {
    ...
    "test_on_testnet": False,       # production endpoint by default
    "scan_interval": 10,
...
class BotController:
    ...
    def is_running(self) -> bool:
        """Return True when the bot is active and not stopping."""
    ...
    # ---- file helpers (thread-safe, sanitized) ----
    ...
    def _sanitize_for_json(self, obj: Any) -> Any:
        """Sanitize objects for JSON write; convert non-serializable pieces to strings."""
        try:
            s = json.dumps(obj, default=str)
            return json.loads(s)
        except Exception:
            # worst-case: convert to str
            try:
                return str(obj)
            except Exception:
                return None
    ...
    def _get_client(self, account: Dict[str, Any]) -> Optional[HTTP]:
        try:
            ...
            if not key or not secret:
                return None
            # pybit unified_trading HTTP: testnet flag toggles endpoints
    ...
    def _capture_preview(self, account: Dict[str, Any], resp: Any):
        """Store a tiny preview of raw responses for debugging (no secrets)."""
        try:
    ...
    # ---- robust SDK caller (non-recursive) ----
    def _try_methods(self, client: HTTP, candidate_names: List[str], *args, **kwargs):
        """
        Try candidate method names on client safely.
        - catches RecursionError
        - ensures response can be serialized (json.dumps default=str)
        - basic Bybit-like error checks
        """
        last_exc = None
        for name in candidate_names:
            ...
            try:
                ...
                last_exc = e
                continue
            # Make sure response is representable
            try:
            ...
            # Basic error checks for Bybit response shapes
            try:
    ...
    # ---- safe ticker/klines and price parsing (visited + depth cap) ----
        ...
        try:
            ...
            if isinstance(raw, dict):
                ...
                for key in ("result", "data", "list", "ticks", "rows", "items"):
                    if key in raw:
                        p = self._parse_price(raw.get(key), depth + 1, visited)
                        if p is not None:
                            return p
                # fallback: try values
                for v in raw.values():
            ...
            if isinstance(raw, (list, tuple)) and raw:
                # kline-like arrays: [ts, open, high, low, close, ...]
                first = raw[0]
                ...
                if len(raw) >= 5 and all(isinstance(x, (int, float, str)) for x in raw[:5]):
                    try:
                        ...
                        pass
                # otherwise try items sequentially
                for item in raw:
    ...
    # ---- balance extraction helpers (UNIFIED -> SPOT -> coin-array) ----
    def _extract_balance_from_payload(self, payload: Any) -> Optional[float]:
        """Best-effort extract USDT-equivalent balance from various payload shapes."""
        try:
            if payload is None:
                return None
            # If dict with totals
                ...
                for k in ("total_equity", "equity", "totalBalance", "total_balance"):
                    if k in payload:
                        try:
                            ...
                            pass
                # sometimes payload contains list of balances
                for list_key in ("list", "balances", "rows", "data", "wallets"):
    ...
    def validate_account(self, account: Dict[str, Any]) -> Tuple[bool, Optional[float], str]:
        """
        Validate account by attempting balances in order:
        1) UNIFIED
        2) SPOT
        3) generic wallet / coin-array
        Returns (ok, balance, error_string)
        """
        client = self._get_client(account)
        if not client:
            return False, None, "missing_api_credentials"
        raw_preview = {}
        # candidate method names and param shapes we will try explicitly (no recursion)
        tries = [
            # unified attempt
            (["get_wallet_balance", "query_wallet_balance"], {"accountType": "UNIFIED"}),
            # spot attempt
            (["get_wallet_balance", "query_wallet_balance"], {"accountType": "SPOT"}),
            # generic wallet (no accountType)
            (["get_wallet_balance", "query_wallet_balance", "get_balances", "balance"], {})
        ]
        balance = None
        last_err = ""
        for candidate_methods, params in tries:
            try:
                # call _try_methods with either params as kwargs or plain call
                try:
                    resp = self._try_methods(client, candidate_methods, **params) if params else self._try_methods(client, candidate_methods)
                except TypeError:
                    # fallback if SDK expects positional or different signature
                    resp = self._try_methods(client, candidate_methods)
                # store a small preview if debug enabled
                try:
                    raw_preview_key = f"attempt_{len(raw_preview)+1}"
                    raw_preview[raw_preview_key] = (json.dumps(resp, default=str)[:400]) if TRADE_SETTINGS.get("debug_raw_responses", False) else None
                except Exception:
                    raw_preview[raw_preview_key] = "preview_error"
                # try to extract balance
                # resp may be dict with 'result' or 'data'
                payload_candidates = []
                if isinstance(resp, dict):
                    if "result" in resp:
                        payload_candidates.append(resp["result"])
                    if "data" in resp:
                        payload_candidates.append(resp["data"])
                    payload_candidates.append(resp)
                else:
                    payload_candidates.append(resp)
                ...
                last_err = str(e)
                continue
        # attach preview (no secrets)
        if TRADE_SETTINGS.get("debug_raw_responses", False) and raw_preview:
            try:
                account.setdefault("last_raw_preview", {})
                account["last_raw_preview"].update(raw_preview)
    ...
    def _place_market_order(self, client: HTTP, symbol: str, side: str, qty: float, price_hint: Optional[float] = None) -> Dict[str, Any]:
        """Place market order; simulated if dry_run True."""
        if TRADE_SETTINGS.get("dry_run", True):
            self.log(f"[DRY RUN] simulated market order: {side} {symbol} qty={qty}")
    ...
    # ---- core trading logic (open / monitor / close) ----
    def attempt_trade_for_account(self, acct: Dict[str, Any]):
        """Open a conservative Buy if monitoring and no open position exists."""
