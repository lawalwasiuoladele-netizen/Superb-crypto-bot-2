...
class BotController:
    ...
    def _should_sell(self, entry_time, entry_price, current_price):
        fee_pct = 0.2  # total buy+sell fee %
        profit_pct = (current_price - entry_price) / entry_price * 100.0
        net_profit_pct = profit_pct - fee_pct  # adjust for fees

        elapsed = (time.time() - entry_time) / 60.0

        # Stop-loss check
        if net_profit_pct <= -5:
            self.log(f"Stop-loss hit: {net_profit_pct:.2f}% (after fees)")
            return True

        # Tiered take-profits
        if elapsed <= 10 and net_profit_pct >= 10:
            self.log(f"Take profit: 10% tier hit ({net_profit_pct:.2f}%)")
            return True
        if elapsed <= 16 and net_profit_pct >= 5:
            self.log(f"Take profit: 5% tier hit ({net_profit_pct:.2f}%)")
            return True
        if elapsed <= 20 and net_profit_pct >= 2:
            self.log(f"Take profit: 2% tier hit ({net_profit_pct:.2f}%)")
            return True

        # Force-sell after 20 minutes
        if elapsed > 20:
            self.log(f"Force-sell triggered at {net_profit_pct:.2f}% (after fees)")
            return True
