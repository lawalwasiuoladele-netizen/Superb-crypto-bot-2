# bot.py
"""
Superb Crypto Bot - Robust multi-account spot trading controller

Notes:
 - This file is MAINNET-ready by default (TRADE_SETTINGS["test_on_testnet"] = False).
 - Test on testnet first if you want: set test_on_testnet=True and use testnet API keys.
 - Start with a tiny allocation (trade_allocation_pct) when going live.
"""
...
RISK_RULES = {
    ...
    "tp1": (600, 7.0),    # <= 600s => 7%
    "tp2": (1020, 4.0),   # <= 1020s => 4% (applies for 600 < t <= 1020)
...
TRADE_SETTINGS = {
    "test_on_testnet": False,      # set True for testnet
    "trade_allocation_pct": 0.01,  # fraction of equity per trade
    "use_market_order": True,
    "scan_interval": 10,
    "symbol_failure_cooldown": 30  # seconds to skip symbol after repeated errors
}

# ---------------------- BotController ----------------------
class BotController:
    def __init__(self, log_queue: Optional[queue.Queue] = None):
        # Use queue.Queue for compatibility with server SSE
    ...
    def _enqueue_log(self, text: str):
        try:
            line = f"[{datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}] {text}"
    ...
    def validate_account(self, acc: Dict[str, Any], retries: int = 1) -> Tuple[bool, float, Optional[str]]:
        """
        Try to fetch a balance quickly to validate API keys.
        Returns (ok, balance, error_message).
        """
        try:
            ...
            for _ in range(max(1, retries)):
                try:
                    ...
                    if lst:
                        # preferred: totalEquity
                        first = lst[0]
                        if "totalEquity" in first:
                            return True, float(first.get("totalEquity", 0) or 0.0), None
                        # fallback: try to sum any numeric fields that might indicate equity
                        # (best-effort)
                        possible = []
    ...
    def get_ticker_last(self, session: HTTP, symbol: str) -> Optional[float]:
        ...
        # REST fallback (single non-recursive attempt)
        try:
    ...
    def get_balance(self, session: HTTP, acc: Dict[str, Any], retries: int = 2) -> Optional[float]:
        """
        Try to fetch a usable total balance for unified wallet.
        On success updates acc['last_balance'] and persists account.
            ...
            try:
                ...
                if lst:
                    ...
                    if first.get("totalEquity") is not None:
                        total = float(first.get("totalEquity") or 0.0)
                        ...
                        try:
                            ...
                            pass
                        return total
                    # fallback: attempt to derive a numeric sum from fields
                    numeric_vals = []
                    for k, v in first.items():
                        try:
    ...
    def check_exit_condition(self, acc: Dict[str, Any], current_price: float) -> Tuple[bool, float, float]:
        try:
            ...
            if elapsed <= tp1_time and profit_pct >= tp1_pct:
                self._enqueue_log(f"TP1 hit for ****{acc.get('key')[-4:]}: profit={profit_pct:.2f}% at {int(elapsed)}s")
                return True, profit_pct, elapsed

            # TP2
            if tp1_time < elapsed <= tp2_time and profit_pct >= tp2_pct:
                self._enqueue_log(f"TP2 hit for ****{acc.get('key')[-4:]}: profit={profit_pct:.2f}% at {int(elapsed)}s")
                return True, profit_pct, elapsed
    ...
    def _main_loop(self):
        ...
        while not self._stop_event.is_set():
            ...
            # periodic balance update (every 60s)
                    ...
                    if should_exit:
                        ...
                        try:
                            if qty:
                                ...
                                self._enqueue_log("No qty known for sell, skipping API sell (manual intervention needed).")
                        except Exception as e:
