"""
Hardened bot.py for Superb Crypto Bot

Key features:
 - Uses queue.Queue for inter-thread SSE logging.
 - Safe non-recursive logging and safe stringification of objects.
 - validate_account returns (ok, balance, error_msg) and does NOT call log() internally.
 - All Bybit API wrapper calls avoid recursion and catch RecursionError.
 - start() spawns one thread per account (reads accounts.json) - compatible with existing server.py.
 - Keeps original scanning / scoring / entry / exit logic.

SAFETY: Set TRADE_SETTINGS["test_on_testnet"] = True while testing with Bybit TESTNET keys.
"""
...
class BotController:
    ...
    # ---- Minimal enqueue log (very safe; won't call other bot methods) ----
    def _enqueue_log(self, text: str):
        try:
            if not isinstance(text, str):
                text = str(text)
            # single-line SSE-safe
            text = text.replace("\n", " ")
    ...
    # ---- Public logger (safe conversion, then minimal enqueue) ----
    def log(self, msg: Any):
        timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
        message = self._safe_to_string(msg)
        self._enqueue_log(f"[{timestamp}] {message}")

    def _safe_to_string(self, obj: Any, max_len: int = 800) -> str:
        """
        Very conservative conversion to avoid deep traversal / recursion:
         - simple primitives are converted directly
         - dicts produce a shallow mapping of simple values; complex values reduced to their type name
         - lists/tuples report length only
         - other objects reported by type name only
        """
        try:
            if obj is None:
                return "None"
            ...
            if isinstance(obj, dict):
                ...
                for k, v in list(obj.items())[:20]:
                    if isinstance(v, (str, int, float, bool)):
                        ...
                        simple[k] = f"<{type(v).__name__} len={len(v)}>"
                    else:
                        simple[k] = f"<{type(v).__name__}>"
                try:
                    s = json.dumps(simple, ensure_ascii=False)
                    return s if len(s) <= max_len else s[:max_len] + "..."
                except Exception:
            ...
            return f"<{type(obj).__name__} RecursionError>"
        except Exception:
            return f"<{type(obj).__name__} convert_error>"
    ...
    def save_accounts(self, accounts: List[Dict[str, Any]]):
        try:
            ...
            self._enqueue_log(f"[save_accounts] failed: {str(e)[:300]}")
    ...
    def add_trade(self, trade: Dict[str, Any]):
        try:
            ...
            self._enqueue_log(f"[add_trade] failed: {str(e)[:300]}")

    # ---- Bybit session helper ----
    def _make_session(self, acc: Dict[str, Any]) -> HTTP:
        # create a fresh HTTP client instance per account thread
        return HTTP(
    ...
    # ---- Account validation (no internal logging) ----
    def validate_account(self, acc: Dict[str, Any], retries: int = 1) -> Tuple[bool, float, Optional[str]]:
        """
        Returns (ok, balance, error_msg). Does NOT call self.log() internally to avoid recursion.
        """
        try:
            ...
            for attempt in range(max(1, retries)):
                try:
                    ...
                    return False, 0.0, f"RecursionError: {str(re)}"
                except Exception as e:
                    last_err = str(e)
                    time.sleep(0.2)
            return False, 0.0, (last_err or "unknown")
    ...
    # ---- API wrappers (safe, catch RecursionError) ----
    def get_ticker_last(self, session: HTTP, symbol: str) -> Optional[float]:
        try:
            ...
            self._enqueue_log(f"get_ticker_last RecursionError for {symbol}: {str(re)}")
            return None
        except Exception as e:
    ...
    def get_klines(self, session: HTTP, symbol: str, interval: str = "1", limit: int = 50) -> List[Dict[str, Any]]:
        try:
            ...
            self._enqueue_log(f"get_klines RecursionError for {symbol}: {str(re)}")
            return []
        except Exception as e:
        ...
        for attempt in range(retries):
            try:
                ...
                if balance_list:
                    first = balance_list[0]
                    total = float(first.get("totalEquity", 0) or 0.0)
                    # update account dict and persist
                    acc["last_balance"] = total
                    try:
                        self._update_account(acc)
                    except Exception:
                        self._enqueue_log("_update_account failed (non-fatal) while get_balance")
                    return total
                else:
    ...
    def _update_account(self, acc: Dict[str, Any]):
        """
        Persist lightweight changes to the accounts file safely; avoid calling self.log() here.
        """
        try:
            with self._file_lock:
    ...
    def check_entry_condition(self, symbol: str, closes: List[float], current_price: float, volumes: Optional[List[float]] = None) -> bool:
        try:
            ...
            self._enqueue_log(f"⚠️ Error in check_entry_condition {symbol}: {str(e)[:200]}")
            return False
    ...
    # ---- Order placement helpers (Bybit unified_trading) ----
    def place_market_buy(self, session: HTTP, symbol: str, usdt_amount: float):
        try:
            ...
            # return price/qty and a light description (do not stringify deep response)
            return {"price": price, "qty": qty, "resp_type": type(res).__name__}
        except RecursionError as re:
            self._enqueue_log(f"place_market_buy RecursionError for {symbol}: {str(re)}")
            raise
        except Exception as e:
            self._enqueue_log(f"place_market_buy failed for {symbol}: {str(e)[:300]}")
            raise
    ...
    def place_market_sell(self, session: HTTP, symbol: str, qty: float):
        try:
            ...
            self._enqueue_log(f"place_market_sell RecursionError for {symbol}: {str(re)}")
            raise
        except Exception as e:
            self._enqueue_log(f"place_market_sell failed for {symbol}: {str(e)[:300]}")
            raise

    # ---- Account runner (main per-account logic) ----
            ...
            try:
                ...
                if time.time() - last_balance_fetch > 60:
                    try:
                        ...
                        self._enqueue_log("Warning: get_balance raised during periodic fetch (non-fatal).")
                ...
                if acc.get("position") == "open" and acc.get("current_symbol"):
                    ...
                    # occasional log while monitoring (not every loop)
                    if int(elapsed) % 60 == 0:
                        self._enqueue_log(f"Account ****{acc.get('key')[-4:]} | Monitoring {symbol} | elapsed={int(elapsed)}s | profit={round(profit_pct,2)}% | price={price}")
                ...
                # Not in a position -> scan allowed coins and select best candidate
                best_score = -9999.0
                best_symbol = None
                best_meta = None
                ...
                if best_symbol:
                    self._enqueue_log(f"Account ****{acc.get('key')[-4:]} | Best candidate: {best_symbol} score={round(best_score,3)}")
                    ...
                    if candidate_ok:
                        ...
                        if allocated <= 0 or equity <= 0:
                            ...
                            try:
                                self._enqueue_log(f"Placing buy for {best_symbol} using ${round(allocated,4)} allocation.")
                ...
                # sleep until next scan, but allow early exit
    ...
    # ---- Control (compatible with server.py calling bot.start()) ----
    def start(self):
        accounts = self.load_accounts()
        if not accounts:
        ...
        for acc in accounts:
            ...
            acc.setdefault("monitoring", False)
            try:
                # quick connectivity test (don't fail hard)
                try:
                    session = self._make_session(acc)
