"""
Patched bot.py
- Uses queue.Queue (no threading.Queue).
- Defensive pybit import and parsing.
- validate_account() helper for server-side debug.
- Hardened error handling and logging.
"""
import json
import os
import threading
...
# Try to import pybit HTTP client; keep a module-level reference (may be None)
try:
...
class BotController:
    ...
    def _make_session(self, acc: Dict[str, Any]):
        ...
        return PYBIT_HTTP(testnet=bool(testnet_flag), api_key=acc.get("key"), api_secret=acc.get("secret"))
    ...
    def run_account(self, acc: Dict[str, Any]):
        ...
        while not self._stop_event.is_set():
            try:
                    ...
                    if should_exit:
                        ...
                        try:
                            if qty:
                                ...
                                self.log("No qty known for sell, skipping sell API call.")
                        except Exception as e:
                ...
                # sleep until next scan interval, check stop event
    ...
    def start(self):
        ...
        for acc in accounts:
            ...
            acc.setdefault("monitoring", False)
            try:
                # create session but don't fail the whole start if session check fails
                try:
                    _ = self._make_session(acc)
                except Exception as e:
                    self.log(f"Quick connectivity test warning for ****{acc.get('key')[-4:]}: {e}")
