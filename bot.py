"""Superb Crypto Bot Controller (Improved)

Multi-account trading

Compounding with available USDT

Tiered take-profit rules
...
SYMBOL = os.environ.get("SYMBOL", "BTCUSDT") DAILY_TRADE_LIMIT = int(os.environ.get("DAILY_TRADE_LIMIT", "50")) TRADE_TIMEOUT = int(os.environ.get("TRADE_TIMEOUT", str(20*60))) CHECK_INTERVAL = int(os.environ.get("CHECK_INTERVAL", "10"))  # seconds MIN_USDT_BALANCE = float(os.environ.get("MIN_USDT_BALANCE", "5.0")) RATE_LIMIT_SLEEP = float(os.environ.get("RATE_LIMIT_SLEEP", "0.35")) MAX_RETRIES = int(os.environ.get("MAX_RETRIES", "3")) TESTNET = bool(int(os.environ.get("TESTNET", "0"))) STOP_LOSS_PCT = -5.0  # stop-loss fixed at -5%

--- RSI Config ---

RSI_PERIOD = 14 RSI_OVERSOLD = 30 RSI_OVERBOUGHT = 70

class BotController: def init(self, log_queue=None, accounts_file=DEFAULT_ACCOUNTS_FILE): self.log_queue = log_queue self.accounts_file = accounts_file self._stop_event = threading.Event() self._thread = None self._lock = threading.Lock() self._running = False self._trade_count_today = 0 self._trade_count_day = datetime.date.today()
...
# ---------- RSI ----------
def _rsi(self, closes, period=RSI_PERIOD):
...
def _should_sell(self, entry_time, entry_price, current_price):
    ...
    if profit_pct <= STOP_LOSS_PCT:
        self.log(f"Stop-loss hit: {profit_pct:.2f}%")
        return True

    # Tiered take-profit
    if elapsed <= 10 and profit_pct >= 10:
        self.log(f"Take profit (10% tier) hit: {profit_pct:.2f}%")
        return True
    if 10 < elapsed <= 16 and profit_pct >= 5:
        self.log(f"Take profit (5% tier) hit: {profit_pct:.2f}%")
        return True
    if 16 < elapsed <= 20 and profit_pct >= 2:
        self.log(f"Take profit (2% tier) hit: {profit_pct:.2f}%")
        return True

    # Force-sell after 20 minutes
    if elapsed > 20:
        self.log(f"Force-sell: elapsed {elapsed:.1f}m, profit {profit_pct:.2f}%")
        return True

    return False

# ---------- trade loop (rest unchanged) ----------
# ... (same as before)

--- Flask Web UI for account management ---

app = Flask(name) bot = BotController()

@app.route("/accounts", methods=["GET"]) def list_accounts(): return jsonify(bot.load_accounts())

@app.route("/accounts", methods=["POST"]) def add_account(): data = request.json bot.add_account(data) return jsonify({"status": "added"})

@app.route("/accounts/int:idx", methods=["DELETE"]) def remove_account(idx): bot.remove_account_index(idx) return jsonify({"status": "removed"})
