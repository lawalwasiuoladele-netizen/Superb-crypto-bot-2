"""
Patched server.py
- Calls eventlet.monkey_patch() early to avoid worker monkey-patch errors.
- Robust endpoints and per-account debug.
- Streams logs via SSE at /logs and /stream.
"""
# Monkey-patch early for eventlet-based gunicorn worker
try:
    import eventlet
    eventlet.monkey_patch()
except Exception:
    # eventlet not installed or patch failed -> continue without monkey-patching
    pass
...
import bot  # import AFTER monkey_patch
...
def start():
    ...
    try:
        # optional: accept JSON payload to replace accounts
...
def account_debug(acc_index: int):
    ...
    key = acc.get("key") or acc.get("api_key") or ""
    secret = acc.get("secret") or acc.get("api_secret") or ""
    testnet_flag = acc.get("test_on_testnet", False)
    debug_result: Dict[str, Any] = {"note": "Do not share secrets. This may contain raw exchange response fields."}
    try:
        if bot.PYBIT_HTTP is None:
            debug_result["session_error"] = "pybit.unified_trading.HTTP not available"
            return jsonify(debug_result)

        session = bot.PYBIT_HTTP(testnet=bool(testnet_flag), api_key=key, api_secret=secret)
