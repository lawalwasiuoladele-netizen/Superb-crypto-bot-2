"""
Patched server.py for Superb Crypto Bot.

Important: eventlet.monkey_patch() is invoked before importing Flask-related modules
so that gunicorn's eventlet worker won't throw monkey-patching errors.
"""
# MUST monkey patch first (before importing Flask/Werkzeug)
try:
    import eventlet
    eventlet.monkey_patch()
except Exception:
    # if eventlet not installed, continue; server will run without eventlet worker
    pass
...
for p, default in ((ACCOUNTS_FILE, []), (TRADES_FILE, [])):
...
def start():
    ...
    try:
        # allow JSON payload that replaces accounts (convenience for UI)
        if data and isinstance(data, dict) and data.get("accounts"):
            ...
            for a in data.get("accounts") or []:
                na = {
                    ...
                    "key": a.get("key") or a.get("api_key") or "",
                    "secret": a.get("secret") or a.get("api_secret") or "",
                    "last_balance": 0.0,
                    "position": "closed",
                    "monitoring": False,
                    "current_symbol": None,
...
def accounts_data():
    ...
    for acc in accounts:
        try:
            # Prefer BotController.validate_account if present
            if hasattr(bot_controller, "validate_account"):
                try:
                    ok, bal, err = bot_controller.validate_account(acc, retries=1)
                except Exception as ve:
                    ok, bal, err = False, None, f"validate_account error: {ve}"
            else:
                ok, bal, err = False, None, "no validate_account available"
...
# ----------------- Per-account debug endpoint -----------------
@app.route("/account_debug/<int:acc_index>")
def account_debug(acc_index: int):
    accounts = load_json_file(ACCOUNTS_FILE, [])
    if acc_index < 0 or acc_index >= len(accounts):
        return jsonify({"error": "index out of range"}), 400

    acc = accounts[acc_index]
    key = acc.get("key") or acc.get("api_key") or acc.get("apiKey") or ""
    secret = acc.get("secret") or acc.get("api_secret") or acc.get("apiSecret") or ""
    testnet_flag = acc.get("test_on_testnet", False)

    debug_result: Dict[str, Any] = {"note": "Do not share secrets. This output may contain exchange response structure."}

    try:
        if bot.HTTP is None:
            debug_result["session_error"] = "pybit.unified_trading.HTTP not available (import failed)"
        ...
        # tickers sample (small set)
        try:
            sample = []
            allowed = getattr(bot, "ALLOWED_COINS", [])[:6] or ["ADAUSDT", "XRPUSDT", "TRXUSDT", "DOGEUSDT", "CHZUSDT", "VETUSDT"]
            for s in allowed:
                try:
        ...
        debug_result["trace"] = traceback.format_exc()
...
def summary_data():
    ...
    wins = sum(1 for t in trades if t.get("profit_pct", t.get("pnl_pct", 0)) > 0)
    losses = sum(1 for t in trades if t.get("profit_pct", t.get("pnl_pct", 0)) < 0)
    total_pnl = sum(t.get("profit_pct", t.get("pnl_pct", 0)) for t in trades)
    accounts_summary = {}
    for t in trades:
        ...
        if t.get("profit_pct", t.get("pnl_pct", 0)) > 0:
            accounts_summary[acc]["wins"] += 1
        elif t.get("profit_pct", t.get("pnl_pct", 0)) < 0:
            accounts_summary[acc]["losses"] += 1
        accounts_summary[acc]["total_pnl"] += t.get("profit_pct", t.get("pnl_pct", 0))
    return jsonify(
        {
...
if __name__ == "__main__":
    ...
    # debug True for local debugging; in production gunicorn should be used
